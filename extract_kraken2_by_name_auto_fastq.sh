...
# --------------------------------------------------------------------
# extract_kraken2_by_name_auto_fastq.sh  (FASTQ, paired or single; taxid-or-name aware)
#
# Extract FASTQ reads (with qualities) assigned by Kraken2 to a
...
#     as well as single-end <sample>.classified.fq
...
#   results/tools/<N>_kraken2_extracted/<OrganismSafe>/<sample>_R2.fq(.gz) [If Paired]
...
find_best_match() {
  ...
  # If looking for R1, also search for unnumbered single-end output formats
  if [[ "$mate" == "1" ]]; then
    pats+=(
      "$base_dir/**/$sample.classified.fq"
      "$base_dir/**/$sample.classified.fastq"
      "$base_dir/**/$sample.classified.fq.gz"
      "$base_dir/**/$sample.classified.fastq.gz"
    )
  fi
...
for KRAKEN in "$KRAKEN_DIR"/*.kraken; do
  ...
  if [[ -z "$R1" ]]; then
    echo "   WARN: classified FASTQ (R1/Single-End) not found for $SAMPLE, skipping."
    continue
  fi
  ...
  if [[ -n "$R2" ]]; then
    ...
    echo "   Extracted ${COUNT} read(s) for Single-End."
    echo "     SE: $OUT_R1"
  fi
