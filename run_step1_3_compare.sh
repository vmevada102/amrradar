...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] Auto threads: using $threads of ${CPU_TOTAL} total cores"
# ------------------------------
...
# Fresh assemblies dir each run (avoid stale symlinks)
mkdir -p "$OUTDIR/assemblies"
rm -f "$OUTDIR/assemblies/"* 2>/dev/null || true
...
for f in "${ASSEMBLIES[@]}"; do
  ...
  # UPDATED: copy instead of relative symlink (avoids broken links)
  cp "$f" "$OUTDIR/assemblies/${safe}.fasta"
  echo "$OUTDIR/assemblies/${safe}.fasta" >> "$OUTDIR/assembly_files.list"
done

# Validate: must have a header and at least one non-header line with bases
BADLIST="$OUTDIR/assemblies_excluded.txt"; > "$BADLIST"
KEPTLIST="$OUTDIR/assemblies_kept.txt"; > "$KEPTLIST"
VALID_FILES=()

for f in "$OUTDIR"/assemblies/*.fasta; do
  # file must exist and be >0
  if [[ ! -s "$f" ]]; then
    echo "$(basename "$f") [empty file]" >> "$BADLIST"; continue
  fi
  if ! grep -m1 -q '^>' "$f"; then
    echo "$(basename "$f") [no FASTA header]" >> "$BADLIST"; continue
  fi
  if ! awk 'BEGIN{ok=0} /^>/ {next} {if ($0 ~ /[ACGTNacgtn]/) ok=1} END{exit ok?0:1}' "$f"; then
    echo "$(basename "$f") [no sequence lines]" >> "$BADLIST"; continue
  fi
  VALID_FILES+=( "$f" )
  echo "$(basename "$f")" >> "$KEPTLIST"
done
...
run_mashtree() {
  ...
  set +e
  mashtree --num-cpus "$threads" --outtree "$MASHTREE_NWK" "${VALID_FILES[@]}" > "$MASHTREE_LOG" 2>&1
  rc=$?
  set -e
  return $rc
}
...
if [[ $force -eq 1 || ! -f "$MASHTREE_DONE_FLAG" ]]; then
  if (( VALID_COUNT == 1 )); then
    ...
    if run_mashtree; then
      ...
      echo "[$(ts)] [ERROR] Mashtree failed with exit code $?"
...
PY
      ...
      if command -v rapidnj >/dev/null 2>&1; then
        rapidnj -i pd -o t "$OUTDIR/mashtree/mash_dist.phylip" > "$MASHTREE_NWK"
      elif command -v fastme >/dev/null 2>&1; then
