...
# ===============================================================
#  make_samples_tsv.sh  (UPDATED FOR CSV GROUP MAPPING & SINGLE-END)
# ---------------------------------------------------------------
#  Interactive script to generate samples.tsv for paired-end & single-end FASTQ
#  Columns: sample    r1    r2    group
#  (Single-end samples will have 'NA' in the r2 column)
#
...
declare -A GROUP_MAP
if [[ "$use_map" =~ ^[Yy]$ ]]; then
  read -rp "??  Enter mapping CSV file path [e.g., sample_group_mapping.csv]: " MAPPING_FILE
...
# --- Detect FASTQs (Both R1 and SE) ---
tmpfile=$(mktemp)
trap 'rm -f "$tmpfile"' EXIT

# Find all fastq/fq files (compressed or not)
find "$FASTQ_DIR" -type f \( -iname "*.fastq*" -o -iname "*.fq*" \) -print0 > "$tmpfile"
...
while IFS= read -r -d '' FQ; do
  ...
  # Skip R2 files so they aren't processed as single-end reads
  if [[ "$fname" == *_R2* || "$fname" == *_2.fastq* || "$fname" == *_2.fq* ]]; then
    continue
  fi
  ...
  if [[ -z "${ADDED[$sample]:-}" ]]; then
    echo -e "${sample}\t${FQ}\t${R2}\t${group}" >> "$OUT_TSV"
    ADDED["$sample"]=1
  fi
done < "$tmpfile"
