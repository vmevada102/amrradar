...
# Now supports both Paired-End (PE) and Single-End (SE) data
# ==============================================================================
...
Modes & inputs:
  --mode {reads|assembly}      Default: reads
    reads:    sample.tsv (sample  r1  r2  group) + raw/ FASTQs
              (For single-end, r2 should be NA or blank)
...
while IFS=$'\t' read -r sample r1 r2 grp rest; do
  ...
  # Normalize empty/NA R2 to truly empty
  if [[ -z "${r2:-}" || "$r2" == "NA" || "$r2" == "na" || "$r2" == "null" ]]; then
    s_r2["$sample"]=""
  else
    s_r2["$sample"]="$r2"
  fi
...
process_one_sample() {
  ...
  if (( RESUME == 1 )) && [[ -s "$out_report" && -s "$out_kraken" && -f "$done_marker" ]]; then
    ...
    if [[ "$MODE" == "reads" ]]; then
      ...
      # Note: For single-end, r2_in will remain empty if no match is found
      if [[ -z "$r1_in" ]]; then
        shopt -s nullglob
        local cand_r1=( "$READS_DIR"/*"${sample}"*_R1*.fastq.gz "$READS_DIR"/*"${sample}"*_R1*.fastq "$READS_DIR"/*"${sample}"*.fastq.gz "$READS_DIR"/*"${sample}"*.fastq )
        shopt -u nullglob
        [[ -z "$r1_in" ]] && r1_in="${cand_r1[0]:-}"
      fi
      if [[ -n "${s_r2[$sample]}" && -z "$r2_in" ]]; then
        shopt -s nullglob
      ...
      # Handle Single-End output naming differences for Kraken2
      if [[ -z "$r2_in" ]]; then
          class_out="$OUTDIR/${sample}.classified.fq"
          unclass_out="$OUTDIR/${sample}.unclassified.fq"
      fi

      echo "  Running kraken2 on reads..."
      if [[ -n "$r2_in" && -f "$r2_in" ]]; then
          # Paired-End Mode
          if ! kraken2 --db "$KRAKEN_DB" \
            ...
            echo "  ERROR: kraken2 PE failed for $sample (see $logf)."
          ...
          if ! kraken2 --db "$KRAKEN_DB" \
              ...
              --unclassified-out "$unclass_out" \
              "$r1_in" >"$logf" 2>&1; then
            echo "  ERROR: kraken2 SE failed for $sample (see $logf)."
            echo -e "${grp}\t${sample}\t${total_reads}\t${total_bases}\tNA\tNA\tNA\tNA\tNA\tunclassified\t0\t${total_reads}\t0.00\t" > "$sum_out"
            : > "$long_out"; : > "$genus_out"
            touch "$done_marker"
            return 0
