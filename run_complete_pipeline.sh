...
RESUME_FLAG=false
for arg in "$@"; do
    if [[ "$arg" == "-resume" || "$arg" == "--resume" || "$arg" == "-r" ]]; then
        RESUME_FLAG=true
...
# --- Auto-Detect Read Layout (PE vs SE) ---
echo ">> Scanning $READS_DIR for read layout..."
# Checks if any R2 or _2 files exist in the directory
PE_FILES=$(find "$READS_DIR" -maxdepth 1 -type f \( -name "*_R2*" -o -name "*_2.*" \) 2>/dev/null | head -n 1)
if [[ -n "$PE_FILES" ]]; then
    export READ_LAYOUT="PE"
    echo "   -> Detected Paired-End (PE) reads. (Exported READ_LAYOUT=PE)"
else
    export READ_LAYOUT="SE"
    echo "   -> Detected Single-End (SE) reads. (Exported READ_LAYOUT=SE)"
fi
...
if ! check_done "phase1_kraken"; then
    ...
    # Check if we need to tell Kraken to resume
    KRAKEN_RESUME_FLAG=""
    if [[ "$RESUME_FLAG" == "true" ]]; then
        KRAKEN_RESUME_FLAG="--resume"
    fi
