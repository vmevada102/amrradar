...
# Now supports both Paired-End (PE) and Single-End (SE) read formats.
#
set -euo pipefail
IFS=$'\n\t'
...
Options:
  -i FILE         sample TSV (tab-delimited) header: sample r1 r2 group 
                  (Leave r2 column blank or use 'NA' for single-end samples)
...
run_sample(){
  ...
  local R2="${fields[${colidx[r2]}-1]:-}" # Use default empty if missing completely
  if [[ -z "$R2" || "$R2" == "NA" || "$R2" == "na" || "$R2" == "null" ]]; then R2=""; fi
...
echo "[`date +'%Y-%m-%d %H:%M:%S'`] Tool: Assembly ($ASSEMBLER) (sample: $SAMPLEID)"
  ...
  if [[ "$RESUME" == "true" && -f "$S3_DIR/.done" ]]; then :; else
    if [[ "$ASSEMBLER" == "spades" ]]; then
      ...
      if [[ -n "$R2" ]]; then
        spades.py -1 "$TRIM_R1" -2 "$TRIM_R2" --isolate --phred-offset 33 -o "$S3_DIR/spades_out" --threads "$THREADS" -m "$MEM_GB" > "$S3_DIR/spades.stdout" 2> "$S3_DIR/spades.stderr" || true
      else
        spades.py -s "$TRIM_R1" --isolate --phred-offset 33 -o "$S3_DIR/spades_out" --threads "$THREADS" -m "$MEM_GB" > "$S3_DIR/spades.stdout" 2> "$S3_DIR/spades.stderr" || true
      fi
    ...
    # THIS IS THE CRITICAL LINE THAT ENABLES RESUME FOR THE NEXT RUN
    touch "$S3_DIR/.done"
  fi
